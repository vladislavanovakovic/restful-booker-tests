...
const roomPrice: number = faker.number.int({ min: 100, max: 999 }).toString()
...
test.describe('Creating room', () => {
  ...
  test('Admin user must NOT be able to create new room with the room price less than 1', async ({ page }) => {
    await roomsPage.createRoom(roomName, roomType, isRoomAccesible, 0, roomFeatures)
    const validationMessage = 'must be greater than or equal to 1'
    await expect(roomsPage.errorMessageBox).toBeVisible()
    await expect(roomsPage.errorMessageBox).toContainText(validationMessage)
  })
})
